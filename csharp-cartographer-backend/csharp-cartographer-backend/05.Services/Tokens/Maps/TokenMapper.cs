    ...
    public class TokenMapper : ITokenMapper
    {
        ...
        private static PrimaryKind GetPrimaryKind(in NavToken token)
        {
            ...
            switch (token.Classifications.Final)
            {
                ...
                case "keyword - operator":
                    return PrimaryKind.KeywordOperator;
                case "identifier - keyword":
                    return PrimaryKind.IdentifierKeyword;
                default:
        ...
        private static SemanticRole GetSemanticRole(in NavToken token)
        {
            ...
            switch (token.PrimaryKind)
            {
                case PrimaryKind.Delimiter:
                    semanticRole = GetDelimiterRole(token);
                    break;
                case PrimaryKind.Punctuation:
                    semanticRole = GetPunctuationRole(token);
                    break;
                case PrimaryKind.Operator:
                    semanticRole = GetOperatorRole(token);
                    break;
                case PrimaryKind.Keyword:
                    semanticRole = GetKeywordRole(token);
                    break;
                case PrimaryKind.KeywordOperator:
                    semanticRole = GetKeywordOperatorRole(token);
                    break;
                ...
                case PrimaryKind.Identifier:
                    ...
                    break;
                case PrimaryKind.IdentifierKeyword:
                    semanticRole = GetIdentifierKeywordRole(token);
                    break;
                default:
                    break;
            }
        ...
        private static SemanticRole GetDelimiterRole(in NavToken token)
        {
            if (token.IsParen())
            {
                ...
                if (token.IsLockStatementControlDelimiter())
                    return SemanticRole.LockStatementControlBoundary;
                ...
                if (token.IsPositionalPatternDelimiter())
                    return SemanticRole.PositionalPatternBoundary;
            ...
            if (token.IsBrace())
            {
                ...
                if (token.IsSwitchExpressionBlockDelimiter())
                    return SemanticRole.SwitchExpressionBlockBoundary;

                if (token.IsSwitchStatementBlockDelimiter())
                    return SemanticRole.SwitchStatementBlockBoundary;
        ...
        private static SemanticRole GetPunctuationRole(in NavToken token)
        {
            // --- Separators ---
            if (token.Text is "," or ":")
            {
                ...
                if (token.IsPatternElementSeparator())
                    return SemanticRole.PatternElementSeparator;

                if (token.IsTypeArgumentSeparator())
                ...
                if (token.IsSwitchArmSeparator())
                    return SemanticRole.SwitchArmSeparator;

                if (token.IsTupleElementSeparator())
                    return SemanticRole.TupleElementSeparator;

                if (token.IsTypeParameterConstraintClauseSeparator())
            ...
            if (token.Text is ";" or ":")
            {
                ...
                if (token.IsCaseSwitchLabelTerminator())
                    return SemanticRole.CaseLabelTerminator;

                if (token.IsCasePatternSwitchLabelTerminator())
        ...
        private static SemanticRole GetOperatorRole(in NavToken token)
        {
            // Arithmetic: +, -, *, /, %, ++, --
            ...
            // Namespace alias: ::
            if (token.IsNamespaceAliasOperator())
                return SemanticRole.NamespaceAlias;
            ...
            // Range / Slice: ..
            if (token.IsRangeSliceOperator())
                return SemanticRole.RangeSlice;
        ...
        private static SemanticRole GetKeywordRole(in NavToken token)
        {
            ...
            if (token.IsRegularKeyword())
            {
                ...
                if (token.IsUsingDirectiveModifierKeyword())
                    return SemanticRole.UsingDirectiveModifier;
                ...
                // --- Resource management ---
                if (token.IsResourceManagementKeyword())
                    return SemanticRole.ResourceManagement;
        ...
        private static SemanticRole TryGetIdentifierRole(in NavToken token)
        {
            ...
            if (token.IsGenericMethodDeclaration())
                return SemanticRole.GenericMethodDeclaration;
